...
export default function AdminDashboard() {
  ...
  const handleDoctorSearch = async () => {
    ...
    try {
      ...
      // Since backend only supports doctorId search, we'll use that
  ...
  const handlePatientSearch = async () => {
    ...
    try {
      ...
      // Since backend only supports patientId search, we'll use that
  ...
  const handleApproveDoctor = async (userId) => {
    try {
      ...
      alert('✅ Doctor approved successfully! Activation email has been sent.');
          ...
          >
            <div className="flex flex-col lg:flex-row gap-6">
              ...
              <div className="flex-1 space-y-3">
                ...
                {doctor.verificationDocPath && (
                  <a
                    href={doctor.verificationDocPath}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 text-[#18AAB0] hover:text-[#86C443] font-medium text-sm"
                  >
              ...
              <div className="flex lg:flex-col gap-3">
                <button
                  ...
                  disabled={loading}
                  className="flex-1 lg:flex-none px-6 py-3 bg-gradient-to-r from-[#18AAB0] to-[#86C443] text-white rounded-xl font-semibold hover:shadow-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                >
                ...
                <button
                  ...
                  disabled={loading}
                  className="flex-1 lg:flex-none px-6 py-3 bg-red-50 text-red-600 border-2 border-red-200 rounded-xl font-semibold hover:bg-red-100 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  ✗ Reject
                </button>
              </div>
...
// Search Type Button
function SearchTypeButton({ active, onClick, children }) {
  return (
    <button
      onClick={onClick}
      className={`px-4 py-2 rounded-lg font-medium text-sm transition-all ${
        active
          ? 'bg-[#18AAB0] text-white'
...
function InfoItem({ label, value }) {
  return (
    <div>
      ...
      <span className="ml-2 text-[#0F4F52] font-medium">{value}</span>
    </div>
  );
}
