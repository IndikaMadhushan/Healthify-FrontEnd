...
export default function ClinicBookPrescriptionPage() {
  ...
  // 🔹 HARD-CODED DUMMY CLINIC BOOK (UI TEST)
  const book = {
    id: 1,
    doctorName: "Amal Perera",
    doctorNo: "SLMC-1023",
    specialization: "Cardiologist",
    medicationPurpose: "Heart checkup & medication review",
    access: "ACCESS_ALL",
    lastUpdated: new Date(),
  };

  // 🔐 TEMP ROLE
  const userRole = "DOCTOR"; // change to "PATIENT" to test UI
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-2xl shadow border p-6">
              ...
              <div className="space-y-3 text-sm text-gray-700">
                ...
                <p><b>Access:</b> {book.access}</p>
                <p className="text-xs text-gray-500">
                  Last updated: {book.lastUpdated.toLocaleDateString()}
                </p>
              </div>
              ...
              {userRole === "DOCTOR" && (
                <button
                  onClick={() => alert("Create Today Page (UI only)")}
                  className="
                    mt-6 w-full flex items-center justify-center gap-2
                    px-4 py-3 rounded-xl
                    bg-gradient-to-r from-[#86c443] to-[#18AAB0]
