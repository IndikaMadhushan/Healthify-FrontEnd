...
//         consultExaming: formData.examinationNotes,
//         consultSuggestTest: formData.suggestedTests,
//         consultDoctorNote: formData.doctorNote,
...
//       if(formData.pulse) metrics["HEART_RATE"] = Number(formData.pulse);
//       if(formData.temperature) metrics["TEMPERATURE"] = Number(formData.temperature);
//       if(formData.weight) metrics["WEIGHT"] = Number(formData.weight);

//       if(Object.keys(metrics).length>0){
//         requestBody.healthMetrics = metrics;
//       }

//       const response = await createConsultPage(patientId,requestBody);

//       const newId = response.data.data.consultId;
...
//       }else{
//         showMessage("Error saving consultation");
//       }
...
//     if(!canEdit){
//       showMessage("Edit window expired");
//       return;
//     }

//     showConfirm(
//       "Do you want to update consultation?",
//       async()=>{
...
//           await updateConsultPage(selectedPageId,requestBody);

//           showMessage("Consultation updated successfully");
...
//             showConfirm(
//               "Edit window expired. Request patient approval?",
//               async()=>{

//                 await requestConsultEditApproval(selectedPageId);
...
//     showConfirm(
//       "Do you want to delete consultation?",
//       async()=>{

//         try{

//           await deleteConsultPage(selectedPageId);

//           showMessage("Consultation deleted");
...
//             showConfirm(
//               "Delete window expired. Request patient approval?",
//               async()=>{
...
//                 onClick={handleComplete}
//                 disabled={isCompleting}
//                 className="ml-auto px-8 py-3 bg-secondary text-white rounded-lg"
//               >
//                 {isCompleting ? "Completing..." : "✅ Complete"}
//               </button>
...
//         consultReason: formData.subReason,
...
//             showConfirm(
//               "Edit time expired. Request patient approval?",
//               async () => {

//                 await requestConsultEditApproval(selectedPageId);

//                 showMessage("Approval request sent");
...
const [formData, setFormData] = useState({
  ...
  showConfirm(
    "Are you sure you want to save? You cannot update or delete later.",
    async () => {
    ...
    setTimeout(() => {
      ...
      // navigate('/patient-dashboard');
        ...
        const metrics = {};
            ...
            if (formData.mediMessure?.BP && formData.mediMessure.BP.includes("/")) {
              const [sys, dia] = formData.mediMessure.BP.split("/");
